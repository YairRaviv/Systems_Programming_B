...
double Graph::getNode_data(std::string name)
{
  double ans = 1;
  if(nodeExists(name))return nodeMap[name]->data;
  return ans;
}
...
//////////////////////////////////////////////////////////num////////////////////////////////////////////////
//ariel::Graph NumberWithUnits::Units = new Graph();
ariel::Graph NumberWithUnits::Units;
NumberWithUnits::NumberWithUnits(double s, string t)
{
    if(!NumberWithUnits::Units.nodeExists(t))throw "not a legal unit in cstr!";
    sum=s;
    type = t;
}
void NumberWithUnits::read_units(ifstream& units_file)
{
                ...
                while(getline(units_file,line))
                {
                    ...
                    //separate the first types and the value
                    unsigned int j = 1;
                    while(line[j]!='=')
                    {
                        type_1+=line[j];
                    ...
                    j++;
                    while(!isalpha(line[j+1])&&line[j+1]!='.')
                    {
                        sum+=(line[j]-'0');
                        sum*=10;
                        j++;
                    ...
                    //check if the units already exist in the graph and update the graph according that with the ratio
                    bool if_1 = NumberWithUnits::Units.nodeExists(type_1)&&NumberWithUnits::Units.nodeExists(type_2);
                    bool if_2 = NumberWithUnits::Units.nodeExists(type_1);
                    bool if_3 = NumberWithUnits::Units.nodeExists(type_2);
                    if(if_1){continue;}
                    else if(if_2)
                    {
                        double data = NumberWithUnits::Units.getNode_data(type_1)/sum;
                        NumberWithUnits::Units.addNode(data,type_2);
                        NumberWithUnits::Units.addEdge(type_1,type_2,1);
                        NumberWithUnits::Units.addEdge(type_2,type_1,1);
                    }
                    else if(if_3)
                    {
                        double data = NumberWithUnits::Units.getNode_data(type_2)*sum;
                        NumberWithUnits::Units.addNode(data,type_1);
                        NumberWithUnits::Units.addEdge(type_1,type_2,1);
                        NumberWithUnits::Units.addEdge(type_2,type_1,1);
                    }
                    else
                    {
                        NumberWithUnits::Units.addNode(1,type_1);
                        NumberWithUnits::Units.addNode(1/sum,type_2);
                        NumberWithUnits::Units.addEdge(type_1,type_2,1);
                        NumberWithUnits::Units.addEdge(type_2,type_1,1);
                    }
                }
...
{
    if(!NumberWithUnits::Units.pathCheck(type,b.type))throw std::invalid_argument("different dimention!!");
    double ratio = NumberWithUnits::Units.getNode_data(type);
    double value = (sum*ratio+b.sum*NumberWithUnits::Units.getNode_data(b.type))/ratio;
...
NumberWithUnits operator*(NumberWithUnits a , double d)
{
   return NumberWithUnits(a.sum*d,a.type);
}
NumberWithUnits operator*(NumberWithUnits a , int d)
{
   return NumberWithUnits(a.sum*d,a.type);
}


////////////////////////////////////////////aritmethic and logic outside class////////////////////////////////

const NumberWithUnits operator+(const NumberWithUnits& a , const NumberWithUnits& b)
{
    if(!NumberWithUnits::Units.pathCheck(a.type,b.type))throw std::invalid_argument("different dimention!!");
    double ratio_a=NumberWithUnits::Units.getNode_data(a.type);
    double ratio_b=NumberWithUnits::Units.getNode_data(b.type);
    double value = (a.sum*ratio_a+b.sum*ratio_b)/ratio_a;
...
{
    return NumberWithUnits(a.sum,a.type);
}
const NumberWithUnits operator-(NumberWithUnits a)
{
    return NumberWithUnits(-a.sum,a.type);
...
NumberWithUnits operator*(double d,NumberWithUnits a)
{
    return NumberWithUnits(a.sum*d,a.type);
}
NumberWithUnits operator*(int d,NumberWithUnits a)
{
    return NumberWithUnits(a.sum*d,a.type);
}
///////////////////////////////////////////comperators outside class////////////////////////////////////////////
bool operator>(const NumberWithUnits& a , const NumberWithUnits& b)
{
    if(!NumberWithUnits::Units.pathCheck(a.type,b.type))throw std::invalid_argument("different dimention!!");
    double ratio_a = NumberWithUnits::Units.getNode_data(a.type);
    double ratio_b = NumberWithUnits::Units.getNode_data(b.type);
    return((a.sum*ratio_a)-(b.sum*ratio_b)>EPS);
}
bool operator>=(const NumberWithUnits& a , const NumberWithUnits& b)
{
    if(!NumberWithUnits::Units.pathCheck(a.type,b.type))throw std::invalid_argument("different dimention!!");
    double ratio_a = NumberWithUnits::Units.getNode_data(a.type);
    double ratio_b = NumberWithUnits::Units.getNode_data(b.type);
    return((a.sum*ratio_a)-(b.sum*ratio_b)>=0);
}
bool operator<(const NumberWithUnits& a , const NumberWithUnits& b)
{
    if(!NumberWithUnits::Units.pathCheck(a.type,b.type))throw std::invalid_argument("different dimention!!");
    double ratio_a = NumberWithUnits::Units.getNode_data(a.type);
    double ratio_b = NumberWithUnits::Units.getNode_data(b.type);
    return((b.sum*ratio_b)-(a.sum*ratio_a)>EPS);
}
bool operator<=(const NumberWithUnits& a , const NumberWithUnits& b)
{
    if(!NumberWithUnits::Units.pathCheck(a.type,b.type))throw std::invalid_argument("different dimention!!");
    double ratio_a = NumberWithUnits::Units.getNode_data(a.type);
    double ratio_b = NumberWithUnits::Units.getNode_data(b.type);
    return((b.sum*ratio_b)-(a.sum*ratio_a)>=0);
}
bool operator==(const NumberWithUnits& a , const NumberWithUnits& b)
{
    if(!NumberWithUnits::Units.pathCheck(a.type,b.type))throw std::invalid_argument("different dimention!!");
    double ratio_a = NumberWithUnits::Units.getNode_data(a.type);
    double ratio_b = NumberWithUnits::Units.getNode_data(b.type);
    return((a.sum*ratio_a-b.sum*ratio_b)<=0.001&&(b.sum*ratio_b-a.sum*ratio_a)<=0.001);
}
bool operator!=(const NumberWithUnits& a , const NumberWithUnits& b)
{
    if(!NumberWithUnits::Units.pathCheck(a.type,b.type))throw std::invalid_argument("different dimention!!");
    double ratio_a = NumberWithUnits::Units.getNode_data(a.type);
    double ratio_b = NumberWithUnits::Units.getNode_data(b.type);
    return(a.sum*ratio_a!=b.sum*ratio_b);
}
...
////////////////////////////////////////////////comperators inside/////////////////////////////////////////
bool NumberWithUnits::operator>(const NumberWithUnits& b )
{
    if(!NumberWithUnits::Units.pathCheck(type,b.type))throw std::invalid_argument("different dimention!!");
    double ratio_a = NumberWithUnits::Units.getNode_data(type);
    double ratio_b = NumberWithUnits::Units.getNode_data(b.type);
    return((sum*ratio_a)-(b.sum*ratio_b)>EPS);
}
bool NumberWithUnits::operator>=(const NumberWithUnits& b )
{
    if(!NumberWithUnits::Units.pathCheck(type,b.type))throw std::invalid_argument("different dimention!!");
    double ratio_a = NumberWithUnits::Units.getNode_data(type);
    double ratio_b = NumberWithUnits::Units.getNode_data(b.type);
    return((sum*ratio_a)-(b.sum*ratio_b)>=0);
}
bool NumberWithUnits::operator<(const NumberWithUnits& b )
{
    if(!NumberWithUnits::Units.pathCheck(type,b.type))throw std::invalid_argument("different dimention!!");
    double ratio_a = NumberWithUnits::Units.getNode_data(type);
    double ratio_b = NumberWithUnits::Units.getNode_data(b.type);
    return((b.sum*ratio_b)-(sum*ratio_a)>EPS);
}
bool NumberWithUnits::operator<=(const NumberWithUnits& b )
{
    if(!NumberWithUnits::Units.pathCheck(type,b.type))throw std::invalid_argument("different dimention!!");
    double ratio_a = NumberWithUnits::Units.getNode_data(type);
    double ratio_b = NumberWithUnits::Units.getNode_data(b.type);
    return((b.sum*ratio_b)-(sum*ratio_a)>=0);
}
bool NumberWithUnits::operator==(const NumberWithUnits& b)
{
    if(!NumberWithUnits::Units.pathCheck(type,b.type))throw std::invalid_argument("different dimention!!");
    double ratio_a = NumberWithUnits::Units.getNode_data(type);
    double ratio_b = NumberWithUnits::Units.getNode_data(b.type);
    return((sum*ratio_a-b.sum*ratio_b)<=0.001&&(b.sum*ratio_b-sum*ratio_a)<=0.001);
}
bool NumberWithUnits::operator!=(const NumberWithUnits& b )
{
    if(!NumberWithUnits::Units.pathCheck(type,b.type))throw std::invalid_argument("different dimention!!");
    double ratio_a = NumberWithUnits::Units.getNode_data(type);
    double ratio_b = NumberWithUnits::Units.getNode_data(b.type);
    return(sum*ratio_a!=b.sum*ratio_b);
}

//set object
istream& operator>>(istream& os , NumberWithUnits& a)
{
    if(!NumberWithUnits::Units.nodeExists(a.type))throw "not a legal unit in >>!";
    string values="";
    char c;
    while(os.get(c))
    {
    ...
    if(!NumberWithUnits::Units.nodeExists(type))
    {
      cout<<"the current input is:"<<values<<"\nthe type is :"<<type<<endl;
      throw "is not a legal unit in end of >>!";
    }
    if(pos==false)
    {
      a.sum = -1*sum;
...
{
    string ans="";
    if(a.sum==DBL_MAX||a.sum==DBL_MIN)
    {
        if(a.sum==DBL_MAX)
        {
        ans+="1.79769e+308";
        }
        else if(a.sum==DBL_MIN)
        {
            ans+="2.22507e-308";
        }
    }
    else
    {
        if(a.sum==(int)a.sum)
        {
            ans+=to_string((int)a.sum);
        }
        else
        {
            string tmp = to_string(a.sum);
            unsigned int i = 0;
            while(tmp[i]!='.')
            {
                ans+=tmp[i];
